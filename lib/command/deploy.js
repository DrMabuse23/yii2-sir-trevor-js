    ...
    if (validation === true) {
        ...
        var uploadConfig = {
            progress: function (percent) {
                ...
                if (percent >= 100) {
                    console.log('\nCreate/Update application on ' + server.name);
                }
            },
...
function deploy(program){
    ...
    mcapDeploy.deploy(config).then(function () {
        console.log('Successfull created Application:\n' + config.baseurl + '/gofer/gui/application.html#application-modify?uuid=' + config.uuid);
    }).catch(function (e) {
        console.log('\nerror', e);
    });
}
